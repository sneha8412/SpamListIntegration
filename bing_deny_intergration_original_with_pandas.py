...
merged_df["SpamDomain"].to_csv("bing_spam_original_domains_from_edge_tobe_added_to_denylist.csv", index=False, header=['origin'])
merged_df.to_csv("bing_original_spammy_domains_in_edge_telemetry.csv", index=False)
...
print("Concatenate: Existing Deny list data frame + New Spammy notificaton domains")
...
concat_deny_list_df.to_csv("final_original_deny_list_domains.csv", index=False, header=None)
...
# TODO : Drop any rows that are empty use df.dropna(inplace=True)
