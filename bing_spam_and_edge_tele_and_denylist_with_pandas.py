...
#merged_df = pd.merge(spam_list_df, telemetry_df, how='inner', on=['NormalizedDomain'])
result_df = merged_df.query("SpamLabel == 'Spam'") # gets all rows with "Spam" label
#print(result_df)
result_df.drop_duplicates(inplace=True) # removes dupes
result_df["SpamDomain"].to_csv("bing_spam_domains_from_edge_tobe_added_to_denylist.csv", index=False, header=['origin'])
result_df.to_csv("bing_spammy_domains_in_edge_telemetry.csv", index=False)
print(f"Spammy Notification Domains Size: {len(result_df.axes[0])}")
...
frames_to_concat = [ deny_list_df, result_df[["SpamDomain"]] ] # put the frames to union in a array
