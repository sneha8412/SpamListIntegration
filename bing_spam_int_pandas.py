...
def FindNotificationSendingDomainsInBingSpamList(bing_spam_domains_csv_file_path, edge_notification_sending_domains_dict):
    ...
    df = pd.read_csv(bing_spam_domains_csv_file_path, usecols=['SpamDomain', 'etldDomain'])

    print(f"Total number of bing spam domains: {df.size}") 

    for index, row in df.iterrows():
        #print(f"index:{index}, Spam Domain:{row['SpamDomain']}, etld Domain:{row['etldDomain']}")
        #print(f"FindNotificationSendingDomainsInBingSpamList: Reading bing spam list domain: {domain_row}")
        sub_domain_name = row['SpamDomain']
        eTLD_name = row['etldDomain']
